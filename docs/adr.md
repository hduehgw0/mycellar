...
- **ステータス**：採用（2026-08-21）
- **文脈**：ADR に無い決定として、#1 で「MVP の割り切り」として 1 つの Neon DB を Production / Preview / Development の 3 環境で共用していた。実データが 0 件の時期の判断としては妥当だったが、#57 でマイグレーションだけが本番に先回りし、**本番のボトル登録が 3 日間 500 で落ちた**。危険が顔を出すのはマイグレーションを打つ瞬間だけで、直前の 19 日間は 1 度も打っていない。**注意では防げない。**
- **決定**：DB を**本番／非本番の 2 つ**に分ける。非本番（Neon の `dev` ブランチ）を Preview とローカルで共用する。#63 で実データを入れる前に実施する。
- **検討した代替案**：デプロイごとに DB ブランチを自動生成（PR 間を隔離できるが、1 人開発では干渉が起きず得るものが少ない）／Preview 専用にもう 1 本（DB が 3 つになり、管理が増える割に前者と同じ理由で利点が薄い）／ローカルを Docker Postgres（フェーズ 4 のスコープ）。
- **結果**：`migrate dev` が本番に届かなくなる。代償として**本番への適用が別手順として必要になる**（暫定は手動の `migrate deploy`。正式な手順と自動化は #98）。Preview は開発用 DB を共有するので、ローカルでの操作が Preview にも見える。
