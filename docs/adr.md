...
- **ステータス**：採用（2026-07-22）
- **文脈**：編集（US-5）で更新エンドポイントの HTTP メソッドを決める必要があった。REST の慣習では PUT は「リソース全体の置換」で冪等性が要求され、PATCH は「部分更新」を表す。書き込みは Route Handler で明示実装する方針（ADR-0002）のため、メソッドの意味論は自分で選ぶことになる。
