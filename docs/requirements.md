...
- **どう解くか**：所有ボトルを記録し、**何を・どれだけ・いくら分**を一覧と傾向で見えるようにする。記録する項目は国・年数・樽・限定版・本数・購入額。
...
| 語                 | 定義                                                                                                                                                   |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **コレクション**   | 所有ボトル全体を指す**画面上の呼称**。コード上は製品を `Whisky`、1 本を `UserBottle` と呼ぶ                                                            |
| **製品名**         | ラベルどおりのフル製品名（「ザ・マッカラン 12年 シェリーオークカスク」）。年数や表現を含むことがある。唯一の必須項目。一覧・詳細の見出しにもこれを使う |
| **同じ製品**       | 製品名が同じこと。大文字小文字・全角半角・空白の違いは同じとみなす → ADR-0016                                                                          |
| **本数**           | 持っているボトルの数。同じ製品を 3 本持っていれば 3。**種類数ではない**                                                                                |
| **種類数**         | 持っている製品の数。同じ製品を何本持っていても 1 と数える。**本数ではない**。傾向ページでは「種類」と表示する                                          |
| **国**             | スコットランド・日本 など。スコットランドとイングランドは別の国として扱う（→ ADR-0017）。固定リストからの選択式。傾向ページの「国別」はこの粒度を指す  |
| **地域**           | アイラ・スペイサイド など、スコットランドの地域区分。固定リストからの選択式で、国がスコットランドのときだけ入力できる                                  |
| **樽**             | 熟成に使った樽の種類（バーボン樽・シェリー樽 等）。固定リストからの選択式                                                                              |
| **限定版**         | 通常のラインナップでない販売品。**判定は父の主観に委ねる**（アプリは真偽値だけを持つ）                                                                 |
| **NAS**            | No Age Statement＝年数表記のないボトル。本アプリは**年数の空欄を NAS として扱い、未入力と区別しない**（区別は #121）                                   |
| **メモ**           | その**製品**についての記録。1 本ごとの記録ではない                                                                                                     |
| **5 大ウイスキー** | スコッチ（スコットランド）／アイリッシュ（アイルランド）／アメリカン（アメリカ）／カナディアン（カナダ）／ジャパニーズ（日本）                         |
| **ニューワールド** | 5 大ウイスキー以外の国。DB には保存せず、国からプログラムで判定する → ADR-0009                                                                         |
...
- **入力負担**：必須は製品名のみ。最悪「名前だけ」で **5 秒で登録**できること
...
- **記録項目**（リッチだが必須は最小）
  - 製品名 …**必須**
  - 国 / 地域（国がスコットランドのときのみ）/ 年数（NAS 可）/ 樽 / 限定版 / メモ …**すべて任意**。国・地域・樽は固定リストからの選択式
  - 本数は項目として持たず、持っているボトルの数で表す（→ ADR-0015）。登録時に何本まとめて登録できるかは #123 で決める
- **重複した製品の検知**：**製品名**が同じなら同じ製品とみなす（国・地域・年数・樽・限定版・メモは含めない）。表記ゆれは NFKC 変換・小文字化・空白除去で吸収し、比較用の列を持って **`userId` との複合一意制約**で保証する（ユーザーをまたいだ一意にはしない → ADR-0016）。**登録・編集のどちらでも検知**し、既存の製品を示して**辿れるようにする**。同じ製品の 2 本目以降は、既存の製品から追加する
- **傾向（簡単な可視化）**：総本数・種類数・国の数のサマリ／国別の本数／限定版の割合。国が未設定のボトルは「未設定」としてグラフに出す（除外しない）
- **UI をモックに合わせる**：完了条件は `docs/ui-mockups/` のモックに一致すること（テーブル分割に合わせた作り直しは #123）
  - 下部タブバー（コレクション・傾向・登録・アカウント）※ナビゲーションは `layout.tsx` に置き、各ページは自分の幅を持たない（後のレスポンシブ対応を layout 1 枚に閉じるため）
  - 一覧は製品単位の 2 カラムカードグリッドで、開くとその製品のボトルが 1 本ずつ並ぶ。カードに出す項目は #123 で決める
  - 0 件のときは空状態を出す
...
| 参照テーブル＋コンボボックス（国・蒸留所の選択肢をユーザーが足せるようにする） | 材料不足：MVP は固定リストで足りている。リストに無い国を登録したい場面がまだ実際に出ていない                                                                   | 固定リストに無い国・蒸留所を登録したくなったとき・蒸留所の項目追加に着手するとき                                             |
| 重複時にその場で 1 本追加する                                                  | 材料不足：MVP は既存の製品へ辿れれば成立する。その手数を父が不便と感じるかが未検証。製品名の修正で既存と重なったときの統合は #122                              | 父のフィードバック・重複登録が実際に頻発したとき                                                                             |
...
| ボトルの状態（未開封・開封済み・空）                                           | 材料不足：何に使うかが固まっていない。残量・開封の管理そのものは除外済み                                                                                       | 購入機能やテイスティングノートの設計に着手するとき                                                                           |
...
| スコッチ／アイリッシュ等への呼称変換・ニューワールド集約                       | 材料不足：モックの傾向ページは国名をそのまま出しており、変換した呼称の置き場がない（ADR-0009 のこの部分は未実装）                                              | 呼称や集約が実際に欲しくなったとき                                                                                           |
| NAS 表示（年数が空のとき「NAS」と出す）                                        | 材料不足：父が年数をどれだけ入力するか未検証。空欄が NAS なのか入力漏れなのかを実データで見たい。区別の持ち方は #121                                           | 父のフィードバック・実データで空欄の割合が分かったとき                                                                       |
