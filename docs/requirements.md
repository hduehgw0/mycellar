...
| **産地**           | 国（スコットランド・日本 など）。固定リストからの選択式。傾向ページの「産地別」はこの粒度を指す                                                        |
| **地域**           | アイラ・スペイサイド など、スコットランドの地域区分。固定リストからの選択式で、産地がスコットランドのときだけ入力できる                                |
...
- **記録項目**（リッチだが必須は最小）
  - 製品名 …**必須**
  - 産地 / 地域（産地がスコットランドのときのみ）/ 年数（NAS 可）/ 樽 / 限定版 / 本数（既定 1）/ メモ …**すべて任意**。産地・地域・樽は固定リストからの選択式
- **重複した製品の検知**：**製品名**が同じなら同じ製品とみなす（産地・地域・年数・樽・限定版・メモは含めない）。表記ゆれは NFKC 変換・小文字化・空白除去で吸収し、比較用の列を持って **`userId` との複合一意制約**で保証する（ユーザーをまたいだ一意にはしない → ADR-0016）。**登録・編集のどちらでも検知**し、既存の製品を示して**辿れるようにする**。同じ製品の 2 本目以降は、既存の製品から追加する
- **傾向（簡単な可視化）**：総本数・種類数・産地数のサマリ／産地別の本数／限定版の割合。産地が未設定のボトルは「未設定」としてグラフに出す（除外しない）
