...
そこで 4 項目を**区切り文字でつないだ**文字列（銘柄名␟年数␟樽␟限定版）を別の列に持ち、**`userId` との複合で一意制約を張る**。つなぐ前に NFKC 変換・空白除去（**内部の空白も含めて全て**）・小文字化を掛けて表記ゆれを吸収し、空欄は空文字にする。NULL が消えるので比較が効く。

区切り文字を省くと、銘柄名「山崎12」＋年数なしと、銘柄名「山崎」＋12 年が同じ文字列になる。ただし**区切りには入力に現れない文字を使い、その文字が来たら共有スキーマで弾く**。銘柄名と樽は自由入力で、API には UI を通さずに送れるため、「入力に現れないはず」という前提だけでは衝突を防げない。`userId` を外すと、誰かが登録済みのボトルを他の人が登録できなくなる。
