...
| `docs/ui-mockups/`     | MVP の UI モック 10 枚（＝「UI をモックに合わせる」の完了条件）。**正本は [Claude Design](https://claude.ai/design/p/e08aa5bb-5c5d-41ae-b16f-1455754fbe9f)**（色・書体・角丸の実値はここから採る。PNG から拾わない）。`assets/` はモックが使う SVG で**設計資料**（ここから import しない。実装時に `src/` へ移す） |
