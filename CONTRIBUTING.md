...
- **コメントはコードが答えないことだけ**：書く／書かないは上から順に判定し、最初に当てはまったものを採る。
  1. 識別子・型・シグネチャを読めば分かる → **書かない**（既にあるなら消す）
  2. 名前や構造を変えれば分かるようになる → コメントではなく**コードを直す**
  3. 同じ説明が他にもある（ADR・Issue・`docs/`・別ファイル）→ **参照 1 行に置き換える**。ただし**その行を触ると壊れる**警告だけは、参照先があっても現場に残す
  4. 残ったものを、削れる語を削って書く

  4 に残るのは主に「**なぜ他のやり方ではなくこうしたか**」「**数値・定数の根拠**」「**知らないと静かに壊れる罠**」の 3 つ。判定例は [#77](https://github.com/hduehgw0/mycellar/issues/77#issuecomment-5248408165)。
